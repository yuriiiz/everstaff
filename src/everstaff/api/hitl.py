...
async def _find_hitl_in_sessions(store, hitl_id: str, index=None) -> tuple[str | None, dict | None, dict | None]:
    ...
    # Fallback: scan all session files (root + sub_sessions)
    try:
    ...
    for path in paths:
        if not (path.endswith("/session.json") or "/sub_sessions/" in path):
            continue
        try:
            raw = await store.read(path)
            session_data = json.loads(raw.decode())
        ...
        for item in session_data.get("hitl_requests", []):
            if item.get("hitl_id") == hitl_id:
                ...
                if not sid:
                    if "/sub_sessions/" in path:
                        sid = path.rsplit("/", 1)[-1].replace(".json", "")
                    else:
                        sid = path.replace("/session.json", "")
...
async def _resolve_hitl_internal(app, hitl_id: str, decision: str, comment=None, grant_scope=None, permission_pattern=None, broadcast_fn=None) -> None:
    ...
    _root_for_path = _entry.root if _entry and _entry.root != session_id else None
        ...
        if index:
            ...
            for path in paths:
                ...
                if session_dirty:
                    ...
                    if not session_id:
                        if "/sub_sessions/" in path:
                            session_id = path.rsplit("/", 1)[-1].replace(".json", "")
                        else:
                            session_id = path.replace("/session.json", "")
                    try:
