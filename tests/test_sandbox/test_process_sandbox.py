...
def _make_sandbox(tmp_path, secret_store=None, **ipc_kwargs):
    """Create a ProcessSandbox with IPC deps configured."""
    sandbox = ProcessSandbox(sessions_dir=tmp_path)
    sandbox.configure_ipc(secret_store=secret_store or SecretStore(), **ipc_kwargs)
    return sandbox
...
async def test_stream_event_callback_wired(tmp_path):
    ...
    sandbox = _make_sandbox(tmp_path, on_stream_event=on_stream)
    await sandbox.start("test-session")
